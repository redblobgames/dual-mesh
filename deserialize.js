...
function deserializeMesh(arraybuffer) {
    ...
    let _triangles = new (uintSizeSides == 2? Int16Array : Int32Array)(arraybuffer, p, numSides);
    p += numSides * uintSizeSides;
    let _halfedges = new (uintSizeOpposites == 2? Int16Array : Int32Array)(arraybuffer, p, numSides);
    p += numSides * uintSizeOpposites;

    // check
    if (p != arraybuffer.byteLength) { throw "miscalculated buffer length"; }
    return {numBoundaryRegions, numSolidSides, _r_vertex, _triangles, _halfedges};
}
