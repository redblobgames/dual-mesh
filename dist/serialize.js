...
function serializeMesh(_ref) {
    ...
    var uintSizeSides = _r_vertex.length < 1 << 16 ? 2 : 4;
    var uintSizeOpposites = _s_start_r.length < 1 << 16 ? 2 : 4;
    var sizeHeader = 4 + 4 + 4 + 4;
    var sizeRegions = _r_vertex.length * 2 * 4;
    var sizeSides = _s_start_r.length * uintSizeSides;
    var sizeOpposites = _s_opposite_s.length * uintSizeOpposites;
    var arraybuffer = new ArrayBuffer(sizeHeader + sizeRegions + sizeSides + sizeOpposites);
    ...
    // region
    var p = 16;
    for (var i = 0; i < _r_vertex.length; i++) {
