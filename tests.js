...
let TriangleMesh = require("./src");
let createMesh = require("./src/create");
let serializeMesh = require("./src/serialize");
let deserializeMesh = require("./src/deserialize");
...
tape("encoding and decoding", function(test) {
  ...
  for (let i = 0; i < 5; i++) {
    for (let j = 0; j < 2; j++) {
      test.ok(
        Math.abs(meshIn._r_vertex[i][j] - meshOut.r_pos([], i)[j]) < 0.01
      );
    }
  }
  test.end();
...
tape("delaunator: properly connected halfedges, random set", function(test) {
  ...
  for (let e1 = 0; e1 < delaunator.halfedges.length; e1++) {
    ...
    if (e2 !== -1 && delaunator.halfedges[e2] !== e1) {
      test.fail(
        "invalid halfedge connection; data set was " + JSON.stringify(points)
      );
    }
  }
